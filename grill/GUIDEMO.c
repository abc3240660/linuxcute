...
void GUIDEMO_SubMenu(void)
{
	...
	do
	{
		...
		if ((evt_val.menu_index != g_event_val_last.menu_index) || evt_val.force_flush) {
			...
			if (5 == g_event_val_new.menu_index) {
				temp_val_center = 180;
			} else if (6 == g_event_val_new.menu_index) {
				temp_val_center = 175;
			...
			if (25 == tmep_val_gap) {
				if (temp_val_center > 450) {
					tmep_val_gap = 5;
				}
				if (temp_val_center < 225) {
					tmep_val_gap = 5;
				}
			}
			...
			if (menu_index_pos1 > 0) {
				if ((evt_val.menu_index > 3) && (evt_val.menu_index < 9)) {
					if (0 == g_temp_val_new.temp_unit) {
						sprintf(dispStr, "%.3d°F", temp_val_center + (tmep_val_gap * 2));
					} else {
						sprintf(dispStr, "%.3d°C", temp_val_center + (tmep_val_gap * 2));
					}
			...
			if (menu_index_pos2 > 0) {
				if ((evt_val.menu_index > 2) && (evt_val.menu_index < 8)) {
					if (0 == g_temp_val_new.temp_unit) {
						sprintf(dispStr, "%.3d°F", temp_val_center + (tmep_val_gap * 1));
					} else {
						sprintf(dispStr, "%.3d°C", temp_val_center + (tmep_val_gap * 1));
					}
		...
		if (EC11_LEFT == ec_sta) {
			if (evt_val.menu_index > 1) {
				if (4 == evt_val.menu_index) {
					if (temp_val_center >= 485) {
						g_event_val_new.menu_index--;
						if (4 == g_event_val_new.menu_index) {
							temp_val_center = 180;
						}
					}
				} else {
				...
				if (g_event_val_new.menu_index < 7) {
					if (temp_val_center <= 495) {
						tmep_val_gap = 5;
						temp_val_center += 5;
						g_event_val_new.force_flush = 1;
					}
			...
			if (evt_val.menu_index < 13) {
				if (4 == evt_val.menu_index) {
					if (temp_val_center <= 185) {
						g_event_val_new.menu_index++;
					}
				} else {
					g_event_val_new.menu_index++;
